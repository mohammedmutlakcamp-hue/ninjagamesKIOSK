...
// ── Install metadata per game id ───────────────────────────────────
// steam = silent install via steam://install/<appid>
// epic  = open Epic Games Launcher store page (user clicks Install)
// url   = open the game's download page in the browser
type InstallSource =
  | { type: 'steam'; appid: number }
  | { type: 'epic';  slug: string }
  | { type: 'url';   url: string };

const INSTALL_SOURCES: Record<string, InstallSource> = {
  // Steam (silent)
  ...
  // Epic-only — opens Epic store page (one click for the worker)
...
function buildShellCommand(src: InstallSource): string {
...
const SOURCE_BADGE: Record<InstallSource['type'], { label: string; color: string }> = {
...
export function RemoteInstallPanel() {
  ...
  const installOn = async (gameId: string, gameLabel: string, src: InstallSource) => {
    ...
    setSending(gameId);
    setLastResult(null);
    const shell = buildShellCommand(src);
