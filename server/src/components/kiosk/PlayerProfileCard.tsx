      ...
      >
        ...
        {loading ? (
          ...
          <>
            ...
            {/* ── Friend Extra Actions (Send Coins / Remove) — sidebar HUD style ── */}
            {isFriend && !isSelf && (
              <div className="px-6 pb-5 space-y-2">
                {/* Send Coins + Remove Friend side-by-side row (matches MESSAGE/CALL layout) */}
                {!confirmRemove && (
                  <div className="flex gap-2">
                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.95 }}
                      onClick={() => { setSendCoinsOpen(!sendCoinsOpen); setSendResult(null); setSendPin(''); setSendPinVerified(false); }}
                      className="relative flex-1 flex items-center justify-center gap-2 py-3 rounded-lg font-ninja text-xs tracking-wider overflow-hidden transition-all"
                      style={{
                        background: 'linear-gradient(135deg, rgba(234,179,8,0.18), rgba(234,179,8,0.05))',
                        border: '1px solid rgba(234,179,8,0.55)',
                        color: '#FFD700',
                        boxShadow: '0 0 14px rgba(234,179,8,0.22)',
                        textShadow: '0 0 6px rgba(234,179,8,0.5)',
                      }}>
                      <div className="absolute top-0 left-0 w-2 h-2" style={{ borderTop: '1.5px solid #FFD700', borderLeft: '1.5px solid #FFD700' }} />
                      <div className="absolute top-0 right-0 w-2 h-2" style={{ borderTop: '1.5px solid #FFD700', borderRight: '1.5px solid #FFD700' }} />
                      <div className="absolute bottom-0 left-0 w-2 h-2" style={{ borderBottom: '1.5px solid #FFD700', borderLeft: '1.5px solid #FFD700' }} />
                      <div className="absolute bottom-0 right-0 w-2 h-2" style={{ borderBottom: '1.5px solid #FFD700', borderRight: '1.5px solid #FFD700' }} />
                    ...
                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.95 }}
                      ...
                      <div className="absolute top-0 right-0 w-2 h-2" style={{ borderTop: '1.5px solid #EF4444', borderRight: '1.5px solid #EF4444' }} />
                      <div className="absolute bottom-0 left-0 w-2 h-2" style={{ borderBottom: '1.5px solid #EF4444', borderLeft: '1.5px solid #EF4444' }} />
                  ...
                  {sendCoinsOpen && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                      <div className="relative rounded-lg p-3 overflow-hidden"
                        ...
                        <p className="font-body text-[10px] text-gray-500 mb-2">10% transfer fee</p>

                        {!sendPinVerified ? (
                          <div className="space-y-2">
                            <NinjaInput type="password" maxLength={6} value={sendPin}
                              onChange={(e) => { setSendPin(e.target.value.replace(/\D/g, '')); setSendResult(null); }}
                              onKeyDown={(e) => (e.key === 'Enter' || e.code === 'NumpadEnter') && handleSendCoins()}
                              placeholder="Enter PIN"
                              icon={<Lock size={16} />}
                              className="text-center tracking-[0.3em]" />
                            <motion.button whileTap={{ scale: 0.97 }} onClick={handleSendCoins} disabled={!sendPin}
                              className="relative w-full py-2 rounded-md font-ninja text-xs tracking-wider disabled:opacity-40 flex items-center justify-center gap-1.5 overflow-hidden transition-all"
                              style={{
                              ...
                              <Lock size={12} /> VERIFY PIN
                            </motion.button>
                          </div>
                        ) : (
                          <div className="flex gap-2">
                            <NinjaInput type="number" value={sendAmount} onChange={(e) => setSendAmount(e.target.value)}
                              onKeyDown={(e) => (e.key === 'Enter' || e.code === 'NumpadEnter') && handleSendCoins()}
                              placeholder="Amount" className="flex-1" autoFocus />
                            <motion.button whileTap={{ scale: 0.93 }} onClick={handleSendCoins} disabled={sendLoading || !sendAmount}
                              className="relative px-4 py-2 rounded-md font-ninja text-xs disabled:opacity-40 flex items-center gap-1 overflow-hidden transition-all"
                              style={{
                                background: 'linear-gradient(135deg, rgba(57,255,20,0.2), rgba(57,255,20,0.06))',
                                border: '1px solid rgba(57,255,20,0.5)',
                                color: '#39FF14',
                        ...
                        {sendResult && <p className={`font-body text-xs mt-2 ${sendResult.includes('Sent') ? 'text-[#39FF14]' : 'text-red-400'}`}>{sendResult}</p>}
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>

                {/* Remove confirmation — replaces the side-by-side row while active */}
                {confirmRemove && (
                  <div className="flex gap-2">
                    <motion.button whileTap={{ scale: 0.95 }} onClick={handleRemoveFriend} disabled={actionLoading}
                      className="relative flex-1 py-3 rounded-lg font-ninja text-xs tracking-wider overflow-hidden flex items-center justify-center gap-2 transition-all"
                      style={{
                      ...
                      <div className="absolute bottom-0 left-0 w-2 h-2" style={{ borderBottom: '1.5px solid #EF4444', borderLeft: '1.5px solid #EF4444' }} />
                      <div className="absolute bottom-0 right-0 w-2 h-2" style={{ borderBottom: '1.5px solid #EF4444', borderRight: '1.5px solid #EF4444' }} />
                      {actionLoading ? <Loader2 size={14} className="animate-spin" /> : 'CONFIRM REMOVE'}
                    </motion.button>
                    <motion.button whileTap={{ scale: 0.95 }} onClick={() => setConfirmRemove(false)}
                      className="px-5 py-3 rounded-lg font-ninja text-xs tracking-wider text-gray-400 hover:text-white transition-all"
